...
from tools.exp_tools import exp_save_object
...
mean_estimator_up = MeanEstimator(sim_up, basis)
mean_est_up = mean_estimator_up.estimate()

# TODO compare output vols

# Save to output file

with mrcfile.new(os.path.join(RESULTS_DIR, "reconstructed70SRibosome_vol_65p.mrc"), overwrite=True) as mrc:
    mrc.set_data(mean_est_up.asnumpy())

# Save real vol and save vol from real rots

exp_save_object(sim, os.path.join(RESULTS_DIR,"sim.pkl"))
exp_save_object(sim_up, os.path.join(RESULTS_DIR,"sim_up.pkl"))
