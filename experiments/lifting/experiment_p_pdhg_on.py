...
from solvers.lifting.update.density.p_pdhg_on import p_primal_dual_hybrid_gradient_update
...
def experiment(exp=None,
    ...
    def dens_update(problem):
        print("================= Start Preconditioned PDHG =================")
        relerrors = p_primal_dual_hybrid_gradient_update(problem, sq_sigma=sq_sigma, regularizer=dens_reg)

        num_its = len(relerrors)

        plt.figure()
        plt.plot(np.arange(num_its), relerrors)
        plt.yscale('log')
        exp.save_fig("p_pdhg_relerrors")
