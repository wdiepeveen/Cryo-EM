...
class Local_Regular(SO3_Multi_Integrator):
    ...
    def __init__(self, quaternions=None, l=9, sep_dist=np.pi / 180, dtype=np.float32):
        assert l>1
        so3 = SO3()
        ...
        super().__init__(quaternions=quaternion_grid, sep_dists=sep_dists, mesh_norms=1 / np.sqrt(2) * sep_dists, tri_dists=np.sqrt(2) * sep_dists,dtype=dtype)
