...
class RefinedMeshIntegrator(Integrator):
    ...
    def __init__(self,
        ...
        self.verts = np.array_split(np.loadtxt(filepath), [4], axis=1)[0]  # used , dtype=self.dtype
        hull = ConvexHull(self.verts)  # We can do this due to the geometry of S3
        ...
        cvertkeep = (cverts.dot(reference_dir) > 0)
        rvertkeep = (rverts.dot(reference_dir) > 0)

        cquaternions = cverts[cvertkeep]
        rquaternions = rverts[rvertkeep]
        # TODO assert that we have the correct amount : divided by 2

        n = rquaternions.shape[0]
        cn = cquaternions.shape[0]

        # Initialize with dummy n
        super().__init__(dtype=dtype, n=n, ell=int((2 * ell_max + 1) * (2 * ell_max + 2) * (2 * ell_max + 3) / 6),
                         t=np.inf)

        self.quaternions = rquaternions
        ...
        # self.b2c = np.real(UV).T.astype(self.dtype)  # TODO asarray?
        # TODO option that we can save the computed arrays in an npy file?
