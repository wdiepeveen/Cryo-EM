...
class RefinedMeshIntegrator(Integrator):
    ...
    def __init__(self,
                 ...
                 mesh_norm=0.285298,  # Maximal length of edges in the triangulation - is 0.285297 # 0.14621 on S3 | was 0.570595
        ...
        # Rots mesh
        angular_resolution = min(mesh_norm, 2 * np.pi/(ell_max + 2))  # second arg is minimum criterion for a quadrature scheme to exist
        self.verts, self.simplices = self.mesh(angular_resolution)
        ...
        V = csc_matrix((Vdata, (Vrow, Vcol)), shape=(self.ell, self.ell), dtype=np.complex64)
    ...
    def mesh_refine(self, verts, simplices, repeat=1):
        ...
        nverts += nsimplices
        print("verts[edges][None] = {}".format(verts[edges]))
        print("shape verts[edges][None] = {}".format(verts[edges].shape))
        verts = np.concatenate((verts,
                                self.mean(verts[edges], np.ones((1, 2)))[0, :, 0],
                                self.mean(verts[simplices], np.ones((1, 4)))[0, :, 0],), axis=0)
        newsims = []
        for j, sim in enumerate(simplices):
    ...
    def mean(self, points, weights, max_iter=200):
        ...
        """
        npointsets, npoints, nintdim = points.shape
        nweights = weights.shape[0]

        if npointsets * nweights * npoints > 100:
            # Log only if it will take a bit longer...
            logger.info(("Computing {N} means of {npoints} "
                          "points in at most {maxiter} steps...").format(
                N=npointsets * nweights,
                npoints=npoints, maxiter=max_iter))

        out = points[:, np.argmax(weights, axis=-1), :].transpose(1, 0, 2)
        # TODO print out and /or check shape
        print(out.shape)
        # Should also be able to compute centre of mass at the end.

        # out_flat = out.reshape((npointsets, nweights, nintdim))
        tmean = out.copy()
        tmean_flat = tmean.reshape((npointsets, nweights, nintdim))
        tpoints = np.zeros((npointsets, nweights, npoints, nintdim))
        tpoints_flat = tpoints.reshape((npointsets * nweights, npoints, nintdim))
        points_flat = points.reshape((npointsets * npoints, nintdim))

        w_sum_inv = 1.0 / np.einsum('kl->k', weights)
        for _iter in range(max_iter):
            self.log(out_flat, points_flat, out=tpoints_flat)
            np.einsum('km,lkmt->lkt', weights, tpoints, out=tmean)
            tmean *= w_sum_inv[:, None, :, None]
            out_flat = self.exp(out_flat, tmean_flat)
