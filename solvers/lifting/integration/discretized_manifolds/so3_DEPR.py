...
class SO3(DiscretizedManifold):
    ...
    def _exp(self, location, vfrom, out):
        """ exp_l(v) = sign(s)*(cos(|v|) * l  + sin(|v|) * v/|v|) for (x,s) := cos(|v|) * l  + sin(|v|) * v/|v| """
        ...
        sign_s = np.sign(out[:, :, :, 0])
        sign_s[sign_s == 0] = 1
        # print("sign_s = {}".format(sign_s))
        out *= sign_s[:,:,:,None]

    def _dist(self, x, y, out):
        np.einsum('ikm,ilm->ikl', x, y, out=out)
        out[:] = np.arccos(np.abs(np.clip(out, -1.0, 1.0)))  # TODO factor 2 in here and rescale log and exp
