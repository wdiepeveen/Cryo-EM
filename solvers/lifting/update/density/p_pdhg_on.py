...
from solvers.lifting.update.density.solvers.p_pdhg import P_PDHG
...
def p_primal_dual_hybrid_gradient_update(problem, sq_sigma=1., regularizer=1.):
    ...
    print("problem.integrator.b2w.T[:,0] = {}".format(problem.integrator.b2w.T[:,0]))
    ...
    def primal_prox(primals, sigmas):
        # print("primals before = {}".format(primals[0,1:10]))
        result = 1/(1 + regularizer * sigmas) * (primals - sigmas * qq[:,1:])
        # print("primals after = {}".format(result[0,1:10]))
        return result

    def dual_prox(duals, taus):
        # print("duals before = {}".format(duals[0,1:10]))
        result = duals + taus/n * np.ones(duals.shape)
        result *= (result <= 0)
    ...
    print("Acola shape = {}".format(Acola.shape))
    Arowa = np.sum(np.abs(A)**alpha, axis=1)    # For the taus
    print("Arowa shape = {}".format(Arowa.shape))
    sigmas = np.repeat(1/Acola[np.newaxis, :], N, axis=0)
    print("sigmas = {}".format(sigmas[0, 0:5]))
    taus = np.repeat(1/Arowa[np.newaxis, :], N, axis=0)
    print("taus = {}".format(taus[0, 0:5]))

    solver = P_PDHG(primal_prox,dual_prox,block_operator,adjoint_block_operator,sigmas,taus,beta,dual_beta) #regularizer)
