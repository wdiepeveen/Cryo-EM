...
def primal_dual_hybrid_gradient_update(problem, sq_sigma=1., regularizer=1.):
    ...
    def block_operator(primals):
        return np.einsum("ij,kj->ik", primals, A)  # TODO check whether we can do this at all with sparse matrix

    def adjoint_block_operator(duals):
        return np.einsum("ij,kj->ik", duals, A.T)  # TODO check whether we can do this at all with sparse matrix
