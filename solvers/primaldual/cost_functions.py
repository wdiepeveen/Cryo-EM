...
def l2_mse_data_fidelity(src):
    # Compute res
    res = src.vol_forward(src.vols, src.rots) - src.data
    # Compute error
    cost = 1 / (2 * src.n) * (2 / src.L) ** 2 * norm(res.asnumpy()) ** 2
    return cost
...
def so3_distance(src, p=2):
    ...
    cost = 1 / p * manifold.dist(src.rots, src.init_rots) ** p
    return cost
