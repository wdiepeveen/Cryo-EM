...
# Own lib
from solvers.primaldual.problem import PrimalDualProblem
...
class PrimalDualSolver(Estimator):
    ...
    def primal_dual_step_g(self):
        ...
        # print("primal update direction = {}".format(-self.sigma * self.adjoint_g(self.problem, m, self.xi_)[0]))

        print("norm tangent vector for primal update step; {}".format(manifold.norm(self.problem.rots, -self.sigma * self.adjoint_g(self.problem, m, self.xi_).asnumpy())))
        # primal update
        new_rots = self.prox_primal_g(self.problem, self.sigma,
                                               manifold.exp(self.problem.rots, -self.sigma * \
                                                            self.adjoint_g(self.problem, m, self.xi_).asnumpy()
                                                            )  # No parallel transport needed on SO(3
                                               )
        # TODO look into error measures
        primal_error = manifold.dist(m, new_rots)
        # dual update
        self.xi = self.prox_dual_g(self.problem, self.tau, Image(
                                   self.xi.data + self.tau * \
                                   self.differential_g(self.problem, m,
                                                        manifold.log(m, new_rots)).asnumpy()
                                                                )
                                   )
        # print("new dual variable xi = {}".format(self.xi[0]))
        ...
        # TODO print the distance between the first rotation and the rot_init at every time step
        man = Rotations(3)
        print("Manifold distance first rotation with initial guess is {}".format(man.dist(self.problem.rots[0], m[0])))
