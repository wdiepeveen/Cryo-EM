...
def compute_c(l, m, n):
    if l == 0:
        ...
        return l/(l*(2*l+1)) * np.sqrt((l**2 - m**2)*(l**2 - n**2))
...
def compute_J2(integrator, quats, coeffs):
    ...
    # TODO also another round for the ell_max+1 indices (also compute D again for these ones) (just vstack on U now)
    ll = integrator.ell_max + 1
    for m in range(-ll+1, ll):
        for n in range(-ll+1, ll):
            index = wigner.Dindex(ll, m, n)
            a = compute_a(ll-1, m, n)
            a_index = wigner.Dindex(ll-1, m, n)
            Crow.append(index)
            Ccol.append(a_index)
            Cdata.append(a)
    ...
    # TODO Note that the only things that change are the values D. The rest should only be computed once!
